...
export class LoggerService implements NestLoggerService {
  private isFirstLogMessage = true;
  private context?: string;
  private lastTimestampAt = 0;
  ...
  protected updateAndGetTimestampDiff(): string {
    const timestampDiff = `${Date.now() - this.lastTimestampAt}`;
    this.lastTimestampAt = Date.now();
    return timestampDiff;
  }

  private updateIsFirstLogMessage() {
    if (this.isFirstLogMessage && this.lastTimestampAt === 0) {
      this.isFirstLogMessage = false;
    }
  }
}
